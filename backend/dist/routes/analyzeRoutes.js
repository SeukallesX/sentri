...
router.post("/", (req, res) => {
    try {
        const message = typeof req.body?.message ===
            "string"
        ...
        if (!message) {
            return res
                .status(400)
                .json({
                error: "A message is required for analysis.",
            });
        }
        if (message.length >
            10000) {
         ...
         * ---------------------------------------
         */
        const result = analyzeMessage(securityEvent.content);
        /*
         * ---------------------------------------
         ...
         * ---------------------------------------
         */
        saveScan("Message", securityEvent.content, resultWithEvent, eventReference);
        /*
         * ---------------------------------------
         * 6. API RESPONSE
         * ---------------------------------------
